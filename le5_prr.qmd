...
<small>In the code below, we calculate the excess return by subtracting the daily risk-free rate from the asset’s return.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>This code below extracts the date and adjusted closing price for Apple, Tesla, and Microsoft, then merges them into one data frame by date.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we create a chart using the Plotly library, which is great for this purpose because it allows you to zoom in, hover over points, and explore the data interactively.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we use the <code>dailyReturn()</code> function from the <code>quantmod</code> package.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we use a line chart to visualize and examine the time series behavior of daily returns for the three selected assets.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we group daily returns into half-year periods and calculate the average (mean) and risk (standard deviation) for each stock in each period.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we add event labels and lines to highlight key periods.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we use <code>plot_ly()</code> to build an interactive chart of volatility trends. Key events are marked to show their impact over time. Alternatively, a <code>ggplot2</code> line chart could be used for a static version, better suited for printed reports.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we create a chart that shows how returns changed over time and highlights the impact of key events. <code>plot_ly()</code> makes it interactive and easy to compare stocks.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the chart, the y-axis shows average daily returns, ranging from -0.4% to +0.6%. This scale helps us quickly see which stocks had positive or negative average performance in each period and how they compare to one another.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we create a chart that compares expected daily return (x-axis) and risk (y-axis), making it easy to assess the risk-return profile of each stock.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we calculate log returns because they measure the relative daily price change. Log returns are preferred in finance since they are additive over time and better handle volatility. Returns (rather than prices) are used for regression and risk analysis.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>The value <code>0.025</code> is a commonly used example of an annual risk-free interest rate. It approximates the long-term yield of safe government bonds, like U.S. Treasuries. The number <code>252</code> reflects the typical number of trading days in a year and is standard in financial modeling.</small>
:::
...
::: {.callout-note appearance="minimal"}
<small>In the code below, we subtract the daily risk-free rate from Apple’s and the S&P 500’s daily returns to calculate the excess return — the amount earned beyond a risk-free investment each day.</small>
:::
...
ggplot(returns, aes(x = SP500_Excess, y = AAPL_Excess)) +
  ...
  labs(title = "AAPL Excess Return vs. S&P 500 Excess Return",
...
cat("Expected return (annual, %):", round(expected_return_percent, 2), "%\n")

```

According to the CAPM model, Apple’s expected annual return is approximately 11–11.5%. This means that, given the current risk-free rate, market risk premium, and Apple’s beta, investors can expect an average annual return of around 11% if they hold Apple stock.
...
```{r}
msft_ret <- dailyReturn(Cl(MSFT), type = "log")
tsla_ret <- dailyReturn(Cl(TSLA), type = "log")
msft_excess <- msft_ret - rf
tsla_excess <- tsla_ret - rf
```

```{r}
capm_model_msft <- lm(msft_excess ~ sp500_excess)
beta_msft <- coef(capm_model_msft)["sp500_excess"]
expected_return_msft <- rf_annual + beta_msft * market_risk_premium
cat("Expected annual return for Microsoft (MSFT):", round(expected_return_msft * 100, 2), "%\n")
```

```{r}
capm_model_tsla <- lm(tsla_excess ~ sp500_excess)
beta_tsla <- coef(capm_model_tsla)["sp500_excess"]
expected_return_tsla <- rf_annual + beta_tsla * market_risk_premium
cat("Expected annual return for Tesla (TSLA):", round(expected_return_tsla * 100, 2), "%\n")

```

Microsoft (MSFT):
Expected annual return = 11.35%
- Indicates moderate risk and stable return, closely aligned with market movements.

Tesla (TSLA):
Expected annual return = 14.32%
...
<div style="background-color:#f0f8ff; border-left:4px solid #64b5f6; padding:10px 14px; font-size:0.9em; color:#333; margin:16px 0;">
  <small>This chart was chosen because it makes it incredibly easy to spot relationships between the stocks at a glance. While we could’ve shown the numbers in a plain table, the color-coded layout provides instant insight without the need to interpret raw values.</small>
</div>
...
Apple vs. Tesla: Which Growth Stock to Own? — TheStreet
https://www.thestreet.com/apple/stock/apple-vs-tesla-which-growth-stock-to-own

Breaking Down the Geometric Mean — Investopedia
